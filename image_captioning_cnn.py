...
if "train_x_128_dask.zarr" in os.listdir('data') and "train_y_dask.zarr" in os.listdir('data'):
    with tf.device('/cpu:0'):
        train_x = da.from_zarr("data/train_x_128_dask.zarr")
        train_y = da.from_zarr("data/train_y_dask.zarr")
else: 
    print("Train dask arrays haven't been saved, please run extract_labels.py")
    exit()

if "validation_x_128_dask.zarr" in os.listdir('data') and "validation_y_dask.zarr" in os.listdir('data'):
    with tf.device('/cpu:0'):
        validation_x = da.from_zarr("data/validation_x_128_dask.zarr")
        validation_y = da.from_zarr("data/validation_y_dask.zarr")
else: 
    print("Train dask arrays haven't been saved, please run extract_labels.py")
    exit()
...
if 'best_model_cnn.h5' in os.listdir('data'):
    model = load_model('data/best_model_cnn.h5')
else:
...
with tf.device('/gpu:0'):
    ...
    ModelCheck = ModelCheckpoint('data/best_model_cnn.h5', monitor='val_loss', mode='min', verbose=1, save_best_only=True)
...
print("\n\nTesting model now, after loading best_model.h5 ....")
testModel = load_model("best_model_cnn.h5")

# image extension *.png,*.jpg
img = Image.open('data/test/COCO_train2014_000000543689.jpg')
